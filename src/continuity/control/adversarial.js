...
import { attributeResult, machineGateStatement, prepareDispatch, settlementDecision } from "./dispatch.js";
import { createScope, gateOverlap } from "./scope.js";
import { interpretCtn } from "./authority.js";
import { createClock } from "./fixture.js";

const GATE_A = { id: "G1", loop: "distribution/launch-post", topic: "launch-post" };
...
export function runAdversarial() {
  ...
  // 1. Gate on topic A, trunk intent concerns topic B -> dispatch allowed.
  const scopeB = createScope({ semantic_centre: "Distribution", topic_refs: ["briefing-surface"] });
  record("gate_on_other_topic_allows_dispatch", "disjoint", gateOverlap(scopeB, GATE_A).overlap);

  // 2. Gate on topic A, intent concerns A -> blocked.
  const scopeA = createScope({ semantic_centre: "Distribution", topic_refs: ["launch-post"] });
  record("gate_on_same_topic_blocks", "intersects", gateOverlap(scopeA, GATE_A).overlap);
  ...
  const scope = createScope({ semantic_centre: "Infra", topic_refs: ["infra"] });
